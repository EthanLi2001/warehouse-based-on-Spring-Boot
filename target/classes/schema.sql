...
ADD COLUMN min_stock INT DEFAULT 0 COMMENT '库存下限' AFTER description,
ADD COLUMN max_stock INT DEFAULT 1000 COMMENT '库存上限' AFTER min_stock;

-- 修改订单表，添加客户ID字段
ALTER TABLE orders 
ADD COLUMN customer_id INT COMMENT '客户ID' AFTER uid;
...
-- 初始化供应商数据
INSERT IGNORE INTO t_supplier (supplier_id, name, contact, phone, address) VALUES 
...
-- 初始化库存数据（假设已有商品数据）
INSERT IGNORE INTO t_inventory (product_id, quantity, min_stock, max_stock, batch_no, warehouse_location) VALUES 
(1, 100, 10, 500, 'B20240101', 'A区-01架'),
(2, 200, 20, 800, 'B20240102', 'A区-02架'),
(3, 50, 5, 300, 'B20240103', 'B区-01架');

SELECT '数据库改造完成' AS result;